...
For class use only. Do not publicly share or post this code without
permission.

Also known as water pouring puzzles, the water jug problem, measuring puzzles
and other similar titles.

It is a basic class of integer problem.

For a fun reference which we use here, see the Die Hard 3 version
...
def pour(jugs, src, dest):
    ...
    Does not modify the jugs - creates a copy.
...
if __name__ == "__main__":
    ...
    if False:
        action_calls = {"fill": fill, "empty": empty, "pour": pour}
        ...
        actions = [
            ...
            ("pour", (0, 1)),  # (5,2) pour jug 1 into jug 2
            ("empty", (1,))  # (4,3) empty jug 1
            # result is be (4,0)
        ]
    ...
    if False:
        ...
        print()
        print(
            "Result: %s (limit=%d, count=%d, history=%d)"
            % (status, limit, count, len(history))
        )
