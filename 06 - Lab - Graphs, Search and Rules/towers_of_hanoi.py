...
def solve_using_recursion(n):
    ...
    def move(n, src, dest, aux):
        if n > 0:
            ...
            print_poles_as_state
            print_poles_as_text

    # generate the recursive sequence stored in moves
    print(
        "Generating set of moves for %d (2^n - 1 = %d) disks using recursion ... "
        % (n, 2**n - 1)
    )
...
def attempt_using_random_moves(n, limit):
    ...
    while status == "Running":
        ...
        if s == s_end:
            ...
            # & The source should be randomly chosen out of the available poles
            # & pick random pole for destination (don't care if it's the src pole)
            # & check if move is valid if not retry.
            _src, _dest = history[-1]

            while True:
                src, dest = sample([0, 1, 2], k=2)
...
if __name__ == "__main__":
    ...
    if False:
        ...
        # ? Note the seed was turned off for these tests
        # ? sequence lengths: [325, 105, 82, 401, 972, 170, 88, 224, 548, 22]

        # ? yeah much better but still not great, though the last attempt only required 8 valid moves

    # & Done
    ### 5 Generate the recursive sequence of moves
    if True:
        solve_using_recursion(n=12)
