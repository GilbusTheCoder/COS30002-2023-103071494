...
class PlanetWars(object):
    def __init__(self, gamestate=None, logger=None, gameid=0, cfg=None):
        ...
        self.excel_logger = ExcelLogger("PlanetWars", "Winners")
    ...
    def add_player(self, name, color=None):
        ...
        if not planets:
            print(
                f"WARNING! There is no home planet in map for player [{player_id}] '{name}'"
            )
            for (
                p_id,
                p,
            ) in self.planets.items():
    ...
    def __str__(self):
        ...
        s = []
        s.append(
            "M %d %d %d %d" % (self.gameid, self.player_id, self.tick, self.winner.id)
        )
        for p in self.planets:
            s.append(
                "P %f %f %d %d %d" % (p.x, p.y, p.owner_id, p.num_ships, p.growth_rate)
            )
        ...
        for p, fleets in arrivals.items():
            ...
            # Battle!
                ...
                if winner_id == 0:  # neutral defense - log nothing
                    ...
                    self.turn_log(
                        "{0:4d}: Player {1} defended planet {2}".format(
                            self.tick, winner_id, p.id
                        )
                    )
                else:
                    self.turn_log(
                        "{0:4d}: Player {1} now owns planet {2}".format(
                            self.tick, winner_id, p.id
                        )
                    )
                # Set the new winner
    ...
    def is_alive(self):
        ...
        if self.winner:
            return False
        # check ...
        winner = [p for p in self.players.values() if p.is_alive()]

        if len(winner) == 1:
            self.winner = winner[0]
            winner[0].log(f"{self.tick:4d}: I won! {winner[0].name} [{winner[0].id}]")
