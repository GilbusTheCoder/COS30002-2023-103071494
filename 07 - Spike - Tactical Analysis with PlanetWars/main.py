...
# player to color map - if more than 6 players, this will need to be extended.
COLOR = {
...
class ScreenPlanet:
    ...
    def _check_update_radii(self):
        ...
        is_change = False
        if self._cache["ratio"] != self.adaptor.ratio:
            is_change = True
        if self._cache["growth_rate"] != self.planet.growth_rate:
            is_change = True
        if self._cache["vision_range"] != self.planet.vision_range:
            is_change = True

        if is_change:
            # set the values
            ratio = self.adaptor.ratio
            self.radius = (ratio * self.PLANET_MIN_R) + (
                (self.PLANET_FACTOR * ratio) * self.planet.growth_rate
            )
            self.view_radius = (
                self.planet.vision_range * ratio
            )  # fog of war vision size
            # note the values
            self._cache.update(
                {
                    "ratio": ratio,
    ...
    def update(self, planet, view_id, view_mode):
        ...
        do_update = False
        if self._cache["view_mode"] != view_mode:
            do_update = True  # pos
        if self._cache["owner_id"] != self.planet.owner_id:
            do_update = True
        ...
        if do_update:
            ...
            # vision circle?
            if view_mode in ["vision_range", "vision_age"] and view_id in [
                0,
                self.planet.owner_id,
            ]:
                if not self.view_circle:
                    self.view_circle = shapes.Circle(
                        ...
                        color=self.VISION_COLOR,
                        batch=self.adaptor.batch_vision,
                    )
                else:
...
class ScreenFleet:
    ...
    def __init__(self, adaptor):
        ...
        self.view_circle = None
        self.label = Label(
            "", color=self.LABEL_COLOR, x=0, y=0, anchor_x="center", anchor_y="center"
        )
        # cache details - see if new shapes needed
    ...
    def _check_update_radii(self):
        ...
        if self._cache["ratio"] != self.adaptor.ratio:
            is_change = True
        if self._cache["vision_range"] != self.fleet.vision_range:
            is_change = True
        ...
        if is_change:
            ...
            # note the values
            self._cache.update(
                {"ratio": ratio, "vision_range": self.fleet.vision_range}
            )
            return True
        else:
            return False  # no change
...
class PlanetWarsScreenAdapter:
    ...
    def sync_all(self, view_id, view_mode):
        ...
        self.screen_planets = {
            k: p for k, p in self.screen_planets.items() if k in planets
        }
        ...
        self.screen_fleets = {
            k: f for k, f in self.screen_fleets.items() if k in fleets
        }
...
class PlanetWarsWindow(window.Window):
    ...
    def __init__(self, **kwargs):
        ...
        self.target_ups = max(int(kwargs.pop("update_target")), self.MIN_UPS)
        self.background_img = kwargs.pop("background_img")
        # self.current_map = kwargs.pop("current_map")
        # self.current_match = kwargs.pop("current_match")
        # self.is_winner_logged = kwargs.pop("is_winner_logged")
        ...
        self.step_label = Label(
            "STEP", x=5, y=self.height - 20, color=COLOR_NAMES["WHITE"]
        )
        self.ups_label = Label(
            "UPS", x=self.width - 10, y=10, color=COLOR_NAMES["WHITE"], anchor_x="right"
        )
        self.mode_label = Label(
            "MODE", x=5, y=self.height - 50, color=COLOR_NAMES["WHITE"]
        )
        self.ups = 0
        self.set_ups(self.target_ups)
        self.view_id = 0
        self.view_mode = "num_ships"

        # create adaptor to help with drawing, and batch for drawing
        self.adaptor = PlanetWarsScreenAdapter(self.game, self.background_img)
    ...
    def update(self, dt):
        ...
        if game:
            ...
            if not game.is_alive() or game.tick >= self.max_tick:
                self.paused = True
    ...
    def add_handlers(self):
        ...
        def on_key_press(symbol, modifiers):
            ...
            # Planet attribute type to show?
                ...
                self.view_mode = (
                    modes[modes.index(i) + 1]
                    if modes.index(i) < (len(modes) - 1)
                    else modes[0]
                )
            # Reset?
            elif symbol == key.R:
...
if __name__ == "__main__":
    ...
    excel_logger = ExcelLogger(
        "PlanetWars6", f"Win Data Matches {current_match} - {max_matches - 1}"
    )

    while current_match < max_matches:
        if current_match % matches_per_map == 0 and not first_map:
            current_map += 1

        map_file = f"./maps/map{current_map}.txt"
        ...
        excel_logger.appendWinData(
            current_match,
            window.game.winner,
            [p for p in window.game.players.values() if not p.is_alive()],
            f"map {current_map}",
        )
